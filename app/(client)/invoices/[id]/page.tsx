...
export default function InvoiceDetailPage({ params }: { params: { id: string } }) {
  ...
  const fetchInvoice = async () => {
    const invoiceRef = ref(database, `invoices/${params.id}`);
    const snapshot = await get(invoiceRef);

    if (snapshot.exists()) {
      const data = { id: params.id, ...snapshot.val() } as Invoice;
      if (data.client_id === user?.id) {
        setInvoice(data);
        // Fetch project info
        if (data.project_id) {
          const projectRef = ref(database, `projects/${data.project_id}`);
          const projectSnapshot = await get(projectRef);
          if (projectSnapshot.exists()) {
            setProject({ id: data.project_id, ...projectSnapshot.val() } as Project);
          }
        }
      } else {
